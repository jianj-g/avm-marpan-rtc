...
static inline void fill_variance_4x4avg(const uint16_t *src_buf, int src_stride,
                                        ...
                                        int pixels_wide, int pixels_high,
                                        int border_offset_4x4, int bit_depth) {
  for (int idx = 0; idx < 4; idx++) {
...
static void fill_variance_tree_leaves(
  ...
  int pixels_wide = is_small_sb ? 64 : 128;
...
int av2_choose_var_based_partitioning(AV2_COMP *cpi, const TileInfo *const tile,
  ...
  PART_EVAL_STATUS force_split[341];
  int avg_64x64;
  ...
  // TODO(any, marpan): check for early exits.
  ...
  if (is_small_sb || mi_col + 32 > tile->mi_col_end ||
    ...
    for (int blk64_idx = 0; blk64_idx < num_64x64_blocks; ++blk64_idx) {
      ...
      if (set_vt_partitioning(cpi, xd, tile, &vt->split[blk64_idx], BLOCK_64X64,
                              mi_row + y64_idx, mi_col + x64_idx, thresholds[2],
      ...
      for (int lvl1_idx = 0; lvl1_idx < 4; ++lvl1_idx) {
        ...
        if (set_vt_partitioning(
                ...
                (mi_col + x64_idx + x32_idx), thresholds[3], BLOCK_16X16,
